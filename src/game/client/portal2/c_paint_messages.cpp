...
bool __MsgFunc_PaintWorld( const CUsrMsg_PaintWorld &msg )
{
	...
	if( !engine->IsClientLocalToActiveServer() )
	{
		...
		C_BaseEntity *pBrushEntity = UTIL_EntityFromUserMessageEHandle( msg.brushentity() );
...
{
	IPaintableEntity* pPaintableEnt = dynamic_cast< IPaintableEntity* >( UTIL_EntityFromUserMessageEHandle( msg.paintableent() ) );
