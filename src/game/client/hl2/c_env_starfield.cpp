...
void C_EnvStarfield::ClientThink( void )
{
	...
	for ( int i = 0; i < iNumParticles; i++ )
	{
		...
		Vector vecEnd = vecViewOrigin + (MainViewRight(GET_ACTIVE_SPLITSCREEN_SLOT()) * RandomFloat(-flDiameter,flDiameter)) + (MainViewUp(GET_ACTIVE_SPLITSCREEN_SLOT()) * RandomFloat(-flDiameter,flDiameter));
