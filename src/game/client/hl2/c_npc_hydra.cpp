...
void C_NPC_Hydra::ResetLatched()
{
	...
	{
		m_iv_vecPos[i].Reset(gpGlobals->curtime);
	}

	BaseClass::ResetLatched();
}

bool C_NPC_Hydra::SetupBones( matrix3x4a_t *pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime )
{
...
void  C_NPC_Hydra::StandardBlendingRules( Vector pos[], Quaternion q[], float currentTime, int boneMask )
{
	...
	if (m_numHydraBones != hdr->numbones())
	{
		...
		//CalcPoseSingle( hdr, NULL, pos, q, 0.0f, 0.0f, poseparam, BONE_USED_BY_ANYTHING );
	...
	if (m_bNewChain)
	{
		...
		//	debugoverlay->AddLineOverlay( m_vecPos[i], m_vecPos[i<m_numHydraBones-1?i+1:m_numHydraBones-1], 0, 255, 0, false, 0.1 );
			...
			{
				m_iv_vecPos[i].NoteChanged( currentTime,currentTime, true );
			}
		}
		m_bNewChain = false;
	}
	...
	if (bNewlyInited)
	{
		...
		for (i = 0; i < m_numHydraBones; i++)
		{
			m_iv_vecPos[i].Reset(currentTime);
		}
	}
