...
void MuzzleFlash_Airboat( ClientEntityHandle_t hEntity, int attachmentIndex )
{
	...
	float flScale = random->RandomFloat( 0.75f, IsGameConsole() ? 2.0f : 2.5f );
	...
	for ( int i = 1; i < 7; i++ )
	{
		...
		pParticle->m_flDieTime		= IsGameConsole() ? 0.0001f : 0.01f;
	...
	if ( muzzleflash_light.GetInt() )
	{
		...
		{
			Vector		origin;
			VMatrix matAtt2(matAttachment);
			MatrixGetColumn(matAtt2, 3, &origin);
