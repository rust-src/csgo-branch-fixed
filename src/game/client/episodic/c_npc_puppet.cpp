...
void C_NPC_Puppet::AccumulateLayers( IBoneSetup &boneSetup, Vector pos[], Quaternion q[], float currentTime )
{
	...
	for (i = 0; i < pTarget->GetNumAnimOverlays(); i++)
	{
		int nOrder = pTarget->GetAnimOverlay(i)->GetOrder();
		if (nOrder < MAX_OVERLAYS)
		{
			layer[nOrder] = i;
		}
	}
	...
	{
		i = layer[ j ];
		if (i >= pTarget->GetNumAnimOverlays())
			continue;

		CAnimationLayer *pTargetLayer = pTarget->GetAnimOverlay(i); 
		float fWeight = pTargetLayer->GetWeight();
		if ( fWeight <= 0.0f )
			continue;

		const char *pSequenceName = pTarget->GetSequenceName( pTargetLayer->GetSequence() );
		int nSequence = LookupSequence( pSequenceName );
		if ( nSequence < 0 )
			continue;

		float fCycle = pTargetLayer->GetCycle();
