...
bool IsPlayerNearTargetPortal(CPortal_Base2D* param_1)
{
	if (sv_portal_turret_shoot_through_portals_proximity.GetFloat() < 0) 
	{
		return true;
	}
	for (int iVar2 = 0; iVar2 < gpGlobals->maxClients; iVar2++ )
	{
		CBasePlayer * this0 = UTIL_PlayerByIndex(iVar2+1);
		if (this0) 
		{
			Vector fVar3 = this0->GetAbsOrigin() - param_1->GetAbsOrigin();
			if (fVar3.x * fVar3.x + fVar3.y * fVar3.y + fVar3.z * fVar3.z < sv_portal_turret_shoot_through_portals_proximity.GetFloat()) 
			{
					return true;
			}
		}
	...
	if ( gpGlobals->curtime > m_fNextTalk && m_iLastState != iNewState )
	{
		...
		switch ( iNewState )
		{
			...
			case PORTAL_TURRET_PICKUP:
				TryEmitSound( GetTurretTalkName( PORTAL_TURRET_PICKUP ) );
...
void CNPC_Portal_FloorTurret::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if ( m_bPickupEnabled && pActivator)
