...
void CPaintDatabase::PreClientUpdate()
{
	...
	for( PaintLocationConstIter currentLocation = cbegin; currentLocation != cend; )
	{
		...
		for( int j = 0; j < paintLocationCount; ++currentLocation, ++j )//theaperturecat
		{
			...
			msg.set_contactpointx(static_cast<short>(offset.x));
			msg.set_contactpointy(static_cast<short>(offset.y));
			msg.set_contactpointz(static_cast<short>(offset.z));
		}
