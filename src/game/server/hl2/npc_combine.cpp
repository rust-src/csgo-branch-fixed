...
void CNPC_Combine::FUN_180139cc0()
{
	...
	if ( m_hForcedGrenadeTarget )
	{
		...
		if (m_hTeleportProjectileForcedTarget) 
		{
			if (CanTossTeleportGrenade(vecTarget))
			{
				...
				//HACK to get tp gren working
				Vector vecStart;
				GetAttachment("lefthand", vecStart);
				m_hTeleportProjectile = Fraggrenade_Create(vecStart, vec3_angle, m_vecTossVelocity, vec3_origin, this, COMBINE_GRENADE_TIMER, true);//CreateNoSpawn("npc_contactgrenade", Weapon_ShootPosition(), vec3_angle, this);
				//m_hTeleportProjectile->KeyValue("velocity", m_vecTossVelocity);
				//m_hTeleportProjectile->Spawn();
...
//In p2ce just is CBaseCombatCharacter::VPhysicsUpdate
