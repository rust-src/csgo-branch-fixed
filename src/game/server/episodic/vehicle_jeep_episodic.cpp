...
void CPropJeepEpisodic::UpdateRadar( bool forceUpdate )
{
	...
	//UserMessageBegin( filter, "UpdateJalopyRadar" ); theaperturecat
	//WRITE_BYTE( 0 ); // end marker
	//MessageEnd();	// send message

	//CUsrMsg_UpdateJalopyRadar msg;
	//msg.se
	//SendUserMessage(filter, UM_UpdateJalopyRadar, msg);
}
...
void CPropJeepEpisodic::InputOutsideTransition( inputdata_t &inputdata )
{
	...
	if ( bSucceeded )
	{
		Teleport( &vecTeleportPos, &vecTeleportAngles, NULL,true );
		return;
	}
