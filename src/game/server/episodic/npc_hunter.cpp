...
void ShootFlechettes( Vector vOrigin, QAngle angles, CBaseEntity *pOwner )
{
	MDLCACHE_CRITICAL_SECTION();

	CBasePlayer *pPlayer = (CBasePlayer*)pOwner;

	CHunterFlechette *entity = CHunterFlechette::FlechetteCreate( vOrigin, angles, pPlayer );
	if ( entity )
	{
		entity->Precache();
...
void CC_Hunter_Shoot_Flechette( const CCommand& args )
{
	...
	ShootFlechettes( vOrigin, angEye, pPlayer );
...
public:
	...
	float			GetDefaultJumpGravity() const		{ return 3.0f; }
...
void CNPC_Hunter::TaskFindDodgeActivity()
{
	...
	while ( !bFoundDir && ( nTries < 2 ) )
	{
		...
		trace_t tr;
		HunterTraceHull_SkipPhysics( GetAbsOrigin(), GetAbsOrigin() + vecDelta, testHullMins, GetHullMaxs(), GetAITraceMask(), this, GetCollisionGroup(), &tr, VPhysicsGetObject()->GetMass() * 0.5f );
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_HUNTER_FIND_SIDESTEP_POSITION:
		{
			...
			else
			{
				...
				trace_t tr;
				UTIL_TraceLine( vecPos, vecPos + Vector( 0, 0, -128 ), GetAITraceMask(), NULL, COLLISION_GROUP_NONE, &tr );
				if ( tr.fraction < 1.0f )
				{
...
float CNPC_Hunter::ChargeSteer()
{
	...
	// Probe
	HunterTraceHull_SkipPhysics( GetAbsOrigin(), testPos, testHullMins, GetHullMaxs(), GetAITraceMask(), this, COLLISION_GROUP_NONE, &tr, VPhysicsGetObject()->GetMass() * 0.5f );
...
void CNPC_Hunter::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( nEvent == AE_HUNTER_FOOTSTEP_LEFT )
	{
		LeftFootHit( pEvent->eventtime );
		return;
	}

	if ( nEvent == AE_HUNTER_FOOTSTEP_RIGHT )
	{
		RightFootHit( pEvent->eventtime );
		return;
	}

	if ( nEvent == AE_HUNTER_FOOTSTEP_BACK )
	{
		BackFootHit( pEvent->eventtime );
		return;
	}
	
	if ( nEvent == AE_HUNTER_START_EXPRESSION )
	{
	...
	if ( nEvent == AE_HUNTER_END_EXPRESSION )
	{
		if ( pEvent->options && Q_strlen( pEvent->options ) )
		{
	...
	if ( nEvent == AE_HUNTER_MELEE_ANNOUNCE )
	{
		EmitSound( "NPC_Hunter.MeleeAnnounce" );
		return;
	}
		
	if ( nEvent == AE_HUNTER_MELEE_ATTACK_LEFT )
	{
	...
	if ( nEvent == AE_HUNTER_MELEE_ATTACK_RIGHT )
	{
		Vector right, forward;
		AngleVectors( GetLocalAngles(), &forward, &right, NULL );
	...
	if ( nEvent == AE_HUNTER_SPRAY_BLOOD )
	{
		Vector vecOrigin;
		Vector vecDir;
...
bool CNPC_Hunter::CanPlantHere( const Vector &vecPos )
{
	...
	trace_t tr;
	UTIL_TraceHull( vecPos, vecPos, vecMins, vecMaxs, GetAITraceMask(), this, COLLISION_GROUP_NONE, &tr );
	if ( tr.startsolid )
	{
		...
		UTIL_TraceHull( vecStart, vecPos, vecMins, vecMaxs, GetAITraceMask(), this, COLLISION_GROUP_NONE, &tr );
	}
...
int CNPC_Hunter::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	trace_t	tr;
	AI_TraceHull( WorldSpaceCenter(), WorldSpaceCenter() + forward * HUNTER_MELEE_REACH, vecMins, vecMaxs, GetAITraceMask(), this, COLLISION_GROUP_NONE, &tr );
...
bool CNPC_Hunter::IsJumpLegal(const Vector &startPos, const Vector &apex, const Vector &endPos) const
{
	...
	trace_t tr;	
	UTIL_TraceHull( startPos, startPos, GetHullMins(), GetHullMaxs(), GetAITraceMask(), this, COLLISION_GROUP_NONE, &tr );
	if ( tr.startsolid )
	{
