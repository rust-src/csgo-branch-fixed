...
public:
	...
	void		Spawn(void);
	void		Precache(void);//PBENCH
	void		Activate(void);

	void		OnSave(IEntitySaveUtils* pUtils); //PBENCH
	void		OnRestore(void);//PBENCH

	Class_T		Classify(void); //PBENCH

	virtual void VPhysicsCollision(int index, gamevcollisionevent_t* pEvent); //PBENCH
	virtual bool CreateVPhysics();//PBENCH
	virtual bool CreateVPhysics(bool bFromRestore);
	virtual int	 VPhysicsGetObjectList(IPhysicsObject** pList, int listMax); //PBENCH
	virtual bool VPhysicsIsFlesh(void); //PBENCH

	// Player pickup
	virtual void OnPhysGunPickup(CBasePlayer* pPhysGunUser, PhysGunPickup_t reason);//PBENCH
	virtual void OnPhysGunDrop(CBasePlayer* pPhysGunUser, PhysGunDrop_t Reason);//PBENCH
	virtual bool OnAttemptPhysGunPickup(CBasePlayer* pPhysGunUser, PhysGunPickup_t reason);//PBENCH

	virtual int  OnTakeDamage_Alive(const CTakeDamageInfo& info);//PBENCH
	virtual bool TestCollision(const Ray_t& ray, unsigned int fContentsMask, trace_t& tr);//PBENCH
	...
	virtual void ApplyDamageForce(const CTakeDamageInfo& info); //PBENCH
